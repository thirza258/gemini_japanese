...
A focused Japanese study website built with React, TypeScript, Vite, and a Node.js/SQLite backend. Features a signature falling sakura petals theme across light mode (sakura pink) and dark mode (night sakura / yozakura). The first visit follows the device preference, and manual choices are saved.

## Study features

- **Landing page & workspace theme:** an introduction to the study tools, an interactive kanji preview, and a starting-level selector. An authentic sakura petal falling theme gracefully drifts across the entire site, with an interactive toggle (flower icon) to enable or disable the falling petals at any time, supporting user preference and reduced-motion settings. The site footer links to each study tool and all five starting levels. “Get started” opens the sidebar workspace; its logo returns to the landing page. Direct links to study activities still work.
